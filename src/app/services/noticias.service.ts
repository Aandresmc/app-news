...
export class NoticiasService {
  ...
  buscarNoticia(busqueda: string, Noticias : Article[]) {
    let noticias: Article[] = [];
    noticias  = Noticias
    let noticiaSearched : Article[] = [];

    Noticias.map((obj, i) => {

        let noticia = noticias[i];
        let buscar = busqueda.toLowerCase();
        let nombre = noticia.title.toLowerCase();
        
        if (nombre.indexOf(buscar) >= 0) {
          noticiaSearched.push(noticia)
        }
    });
    return noticiaSearched
